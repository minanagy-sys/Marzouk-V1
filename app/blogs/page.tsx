...
import { blogsUi, blogPosts } from "@/lib/content/blogs";
          ...
          {posts.map((p) => (
            ...
            >
              <div style={{ height: 200 }}>
                <ImageSlot placeholder={ui.photoPh} style={{ width: "100%", height: 200 }} />
              </div>
              <div style={{ padding: 26, display: "flex", flexDirection: "column", gap: 12, flex: 1 }}>
                <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                  <span style={{ background: "rgba(48,182,222,0.12)", border: "1px solid rgba(48,182,222,0.35)", color: "#1E92B8", borderRadius: 999, padding: "4px 13px", fontSize: 12, fontWeight: 800 }}>{p.tag}</span>
                  <span style={{ fontSize: 12.5, color: "#8AA5B1", fontWeight: 700 }}>{p.date}</span>
                </div>
                <div style={{ fontFamily: SERIF, fontWeight: 700, fontSize: 19, lineHeight: 1.5 }}>{p.title}</div>
                <p style={{ fontSize: 14, lineHeight: 1.9, color: "#5B7A88", margin: 0, flex: 1 }}>{p.excerpt}</p>

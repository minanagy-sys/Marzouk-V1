...
type Pin = ReturnType<typeof contactContent>["pins"][number];

function MapPin({ pin, index, open, onToggle, directions }: { pin: Pin; index: number; open: boolean; onToggle: () => void; directions: string }) {
    ...
    <div style={{ position: "absolute", ...pos }}>
      {open && (
        <div className="dam-tip" style={{ position: "absolute", bottom: 74, left: "50%", transform: "translateX(-50%)", width: 290, background: "#ffffff", border: "1px solid rgba(48,182,222,0.4)", borderRadius: 20, padding: "20px 22px", boxShadow: "0 22px 50px rgba(12,52,70,0.25)", zIndex: 5 }}>
          <div style={{ fontFamily: SERIF, fontWeight: 700, fontSize: 18, color: "#0C3446" }}>{pin.name}</div>
          <div style={{ fontSize: 13, color: "#5B7A88", lineHeight: 1.8, marginTop: 6 }}>{pin.address}</div>
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 12, marginTop: 14 }}>
            <a href={`tel:${pin.phone}`} style={{ fontWeight: 800, fontSize: 14.5, color: "#1E92B8", direction: "ltr" }}>{pin.phone}</a>
            <HoverBox as="a" href={pin.url} target="_blank" rel="noopener noreferrer" style={{ background: "linear-gradient(135deg, #30B6DE, #1E92B8)", color: "#ffffff", borderRadius: 999, padding: "8px 16px", fontSize: 12.5, fontWeight: 800 }} hoverStyle={{ color: "#ffffff" }}>{directions}</HoverBox>
          </div>
      ...
      <button onClick={onToggle} style={{ position: "relative", background: "none", border: "none", cursor: "pointer", padding: 0, display: "flex", flexDirection: "column", alignItems: "center", gap: 8, transform: "translate(-50%, -100%)" }}>
        ...
        <svg width="46" height="58" viewBox="0 0 46 58" fill="none" style={{ position: "relative", filter: "drop-shadow(0 10px 18px rgba(12,52,70,0.35))" }}>
          ...
          <defs>
            <linearGradient id={`damPinGrad${index}`} x1="0" y1="0" x2="46" y2="58">
              <stop offset="0" stopColor="#30B6DE" />
              <stop offset="1" stopColor="#0E5372" />
            </linearGradient>
          </defs>
        </svg>
        <span style={{ background: "rgba(255,255,255,0.94)", border: "1px solid rgba(48,182,222,0.4)", borderRadius: 999, padding: "6px 16px", fontFamily: SANS, fontSize: 12.5, fontWeight: 800, color: "#0C3446", whiteSpace: "nowrap", boxShadow: "0 8px 18px rgba(12,52,70,0.15)" }}>{pin.area}</span>
      </button>
    </div>
  );
}
...
export default function ContactPage() {
  ...
  const { t, clinics, pins } = contactContent(lang);
  ...
  const inputStyle = {
    border: "1.5px solid rgba(12,52,70,0.14)",
    borderRadius: 13,
    padding: "15px 18px",
    fontFamily: SANS,
    fontSize: 15,
      ...
      <section data-screen-label="Clinics map" style={{ padding: "90px 24px 0" }}>
        <div style={{ maxWidth: 1240, margin: "0 auto" }}>
          ...
          <div className="dam-map" style={{ position: "relative", marginTop: 40, borderRadius: 30, overflow: "hidden", border: "1px solid rgba(48,182,222,0.3)", boxShadow: "0 24px 60px rgba(12,52,70,0.14)", height: 520, background: "linear-gradient(160deg, #DDF2FA 0%, #EAF7FB 45%, #F4FBFD 100%)" }}>
            ...
            {pins.map((pin, i) => (
              <MapPin key={i} pin={pin} index={i} open={activePin === i} onToggle={() => setActivePin((p) => (p === i ? -1 : i))} directions={t.directions} />
            ))}
          </div>
        </div>
      </section>
      ...
      <section data-screen-label="Contact form and info" style={{ padding: "90px 24px" }}>
        <div className="dam-2col" style={{ maxWidth: 1240, margin: "0 auto", display: "grid", gridTemplateColumns: "1.1fr 0.9fr", gap: 50, alignItems: "start" }}>
            ...
            {status === "sent" ? (
              ...
              <div className="dam-2col-sm" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 18, marginTop: 28 }}>
                ...
                <HoverBox
                  ...
                  style={{ gridColumn: "span 2", background: "linear-gradient(135deg, #30B6DE, #1E92B8)", color: "#ffffff", border: "none", borderRadius: 999, padding: 16, fontFamily: SANS, fontWeight: 800, fontSize: 16, cursor: status === "sending" ? "wait" : "pointer", boxShadow: "0 10px 26px rgba(48,182,222,0.4)", opacity: status === "sending" ? 0.7 : 1 }}
                  hoverStyle={{ boxShadow: "0 14px 34px rgba(48,182,222,0.55)" }}
                >
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 22 }}>
            {clinics.map((c) => (
              <div key={c.name} style={{ background: "#F4FBFD", border: "1px solid rgba(48,182,222,0.25)", borderRadius: 22, padding: 30 }}>
                <div style={{ fontFamily: SERIF, fontWeight: 700, fontSize: 20, color: "#0C3446" }}>{c.name}</div>
                <div style={{ fontSize: 14.5, lineHeight: 1.9, color: "#5B7A88", marginTop: 10 }}>{c.address}</div>
                <div style={{ fontSize: 13.5, color: "#1E92B8", fontWeight: 700, marginTop: 10 }}>{c.hours}</div>
              </div>
            ))}

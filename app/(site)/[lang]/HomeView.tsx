    ...
    <div style={{ fontFamily: SANS, direction: dir, color: "#0C3446", background: "#ffffff", minHeight: "100vh" }}>
      ...
      <section data-screen-label="Celebrity cases" style={{ padding: "90px 24px", background: "linear-gradient(180deg, #F4FBFD 0%, #ffffff 100%)" }}>
        <div style={{ maxWidth: 1240, margin: "0 auto" }}>
          ...
          <Slider bleed>
            {celebrities.map((cel) => (
              <HoverBox key={cel.slug} as={Link} href={lp(`/cases/${slugFor(cel, lang)}`)} style={{ display: "block", position: "relative", height: 420, borderRadius: 26, overflow: "hidden", border: "1px solid rgba(12,52,70,0.08)", boxShadow: "0 12px 34px rgba(12,52,70,0.12)", background: "linear-gradient(160deg, #0A3950, #0E5372)", color: "#ffffff", transition: "all 0.35s ease" }} hoverStyle={{ transform: "translateY(-8px)", boxShadow: "0 30px 64px rgba(48,182,222,0.32)", color: "#ffffff" }}>

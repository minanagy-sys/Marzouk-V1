    ...
    <div style={{ fontFamily: SANS, direction: dir, color: "#0C3446", background: "#ffffff", minHeight: "100vh" }}>
        ...
        <div style={{ position: "relative", maxWidth: 1240, margin: "0 auto", width: "100%", padding: "120px 24px 60px", flex: 1, display: "flex", alignItems: "center", boxSizing: "border-box" }}>
          <div key={"s" + slide} style={{ maxWidth: 660, animation: slideAnim }}>
            ...
            <div style={{ display: "flex", gap: 16, marginTop: 38, flexWrap: "wrap" }}>
              <HoverBox as={Link} href={lp("/contact")} style={{ background: "linear-gradient(135deg, #30B6DE, #1E92B8)", color: "#ffffff", borderRadius: 999, padding: "17px 38px", fontWeight: 800, fontSize: 16, boxShadow: "0 12px 30px rgba(48,182,222,0.45)" }} hoverStyle={{ boxShadow: "0 16px 38px rgba(48,182,222,0.6)", color: "#ffffff" }}>{t.heroCta1}</HoverBox>
              <HoverBox as={Link} href={lp("/services")} style={{ border: "1.5px solid rgba(255,255,255,0.4)", color: "#ffffff", borderRadius: 999, padding: "17px 38px", fontWeight: 700, fontSize: 16, backdropFilter: "blur(4px)" }} hoverStyle={{ borderColor: "#30B6DE", color: "#8FE0F7" }}>{t.heroCta2}</HoverBox>
            </div>
          </div>
        </div>
      ...
      <section data-screen-label="Services" style={{ padding: "90px 24px", background: "linear-gradient(180deg, #F4FBFD 0%, #ffffff 100%)" }}>
        <div style={{ maxWidth: 1240, margin: "0 auto" }}>
          <SectionHead kicker={t.svcKicker} title={t.svcTitle} href={lp("/services")} cta={t.viewAll} arrow={arrow} />
          <Slider bleed>
            {services.map((svc) => (
      ...
      <section data-screen-label="Why choose us" style={{ padding: "100px 24px" }}>
        <div className="dam-2col" style={{ maxWidth: 1240, margin: "0 auto", display: "grid", gridTemplateColumns: "0.9fr 1.1fr", gap: 60, alignItems: "center" }}>
          <div>
            ...
            <HoverBox as={Link} href={lp("/about")} style={{ display: "inline-flex", marginTop: 30, background: "#0C3446", color: "#ffffff", borderRadius: 999, padding: "14px 30px", fontWeight: 700, fontSize: 15 }} hoverStyle={{ background: "#1E92B8", color: "#ffffff" }}>{t.whyCta}</HoverBox>
          </div>
      ...
      <section data-screen-label="Celebrity cases" style={{ padding: "90px 24px", background: "linear-gradient(180deg, #F4FBFD 0%, #ffffff 100%)" }}>
        <div style={{ maxWidth: 1240, margin: "0 auto" }}>
          <SectionHead kicker={t.celebKicker} title={t.celebTitle} href={lp("/cases#celebs")} cta={t.viewAll} arrow={arrow} />
          <Slider bleed>
            {celebrities.map((cel) => (
              <HoverBox key={cel.slug} as={Link} href={lp(`/cases/${slugFor(cel, lang)}`)} className="celeb-card" style={{ display: "block", position: "relative", height: 420, borderRadius: 26, overflow: "hidden", border: "1px solid rgba(12,52,70,0.08)", boxShadow: "0 12px 34px rgba(12,52,70,0.12)", background: "linear-gradient(160deg, #0A3950, #0E5372)", color: "#ffffff", transition: "all 0.35s ease" }} hoverStyle={{ transform: "translateY(-8px)", boxShadow: "0 30px 64px rgba(48,182,222,0.32)", color: "#ffffff" }}>
      ...
      {posts.length > 0 && (
        <section data-screen-label="News" style={{ padding: "0 24px 100px" }}>
          <div style={{ maxWidth: 1240, margin: "0 auto" }}>
            <SectionHead kicker={t.newsKicker} title={t.newsTitle} href={lp("/blogs")} cta={t.viewAll} arrow={arrow} />
            <Slider bleed>
              {posts.map((n) => (
                <HoverBox key={n.slug} as={Link} href={lp(`/blogs/${slugFor(n, lang)}`)} style={{ flex: "0 0 340px", scrollSnapAlign: "start", background: "#ffffff", border: "1px solid rgba(12,52,70,0.08)", borderRadius: 22, overflow: "hidden", color: "#0C3446", boxShadow: "0 4px 14px rgba(12,52,70,0.04)", display: "flex", flexDirection: "column" }} hoverStyle={{ transform: "translateY(-6px)", boxShadow: "0 20px 44px rgba(48,182,222,0.16)", color: "#0C3446" }}>

...
export default function MediaPage() {
  ...
  const galleryItems: LightboxItem[] = GALLERY_SLOTS.map((g) => ({ src: (g as { imageUrl?: string }).imageUrl, title: undefined }));
  const videoItems: LightboxItem[] = videos.map((v) => ({ videoUrl: (v as { videoUrl?: string }).videoUrl, title: v.title }));
    ...
    <div style={{ fontFamily: SANS, direction: dir, color: "#0C3446", background: "#ffffff", minHeight: "100vh" }}>
      ...
      {tab === "gallery" && (
        <section data-screen-label="Gallery" style={{ padding: "80px 24px" }}>
          <div className="dam-mosaic" style={{ maxWidth: 1240, margin: "0 auto", display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gridAutoRows: "220px", gap: 18 }}>
            {(GALLERY_SLOTS as { id: string; gc: string; gr: string; imageUrl?: string }[]).map((g, i) => (
      ...
      {lb && (
        <Lightbox
          ...
          ytFallback={CONTACT_INFO.youtube}
          watchLabel={t.watchOnYt}
        />
      )}

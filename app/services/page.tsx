...
export default function ServicesPage() {
  ...
  const { t, services } = servicesContent(lang);
      ...
      <section data-screen-label="Services mosaic" style={{ padding: "90px 24px" }}>
        <div className="dam-mosaic" style={{ maxWidth: 1240, margin: "0 auto", display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gridAutoRows: "215px", gap: 20 }}>
          {services.map((svc) => (
            <HoverBox
              key={svc.slotId}
              style={{ position: "relative", borderRadius: 26, overflow: "hidden", border: "1px solid rgba(12,52,70,0.1)", boxShadow: "0 10px 28px rgba(12,52,70,0.08)", gridColumn: svc.gc, gridRow: svc.gr, background: "linear-gradient(160deg, #0A3950, #0E5372)", transition: "all 0.35s ease" }}
              hoverStyle={{ boxShadow: "0 26px 56px rgba(48,182,222,0.3)", borderColor: "rgba(48,182,222,0.5)" }}
            >
              ...
              <span style={{ position: "absolute", top: 18, insetInlineStart: 18, background: "rgba(48,182,222,0.92)", color: "#ffffff", borderRadius: 999, padding: "6px 16px", fontSize: 12, fontWeight: 800, letterSpacing: "0.5px", pointerEvents: "none", boxShadow: "0 6px 16px rgba(4,32,46,0.35)" }}>{svc.tag}</span>
              <span style={{ position: "absolute", bottom: 0, left: 0, right: 0, padding: "24px 24px 22px", pointerEvents: "none", display: "flex", alignItems: "flex-end", justifyContent: "space-between", gap: 12 }}>
                <span style={{ fontFamily: SERIF, fontWeight: 700, fontSize: "clamp(19px, 1.8vw, 25px)", lineHeight: 1.45, color: "#ffffff" }}>{svc.title}</span>
                <HoverBox as={Link} href="/contact" style={{ pointerEvents: "auto", flexShrink: 0, width: 42, height: 42, borderRadius: "50%", background: "rgba(255,255,255,0.14)", border: "1px solid rgba(255,255,255,0.35)", backdropFilter: "blur(6px)", color: "#ffffff", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 17 }} hoverStyle={{ background: "#30B6DE", borderColor: "#30B6DE", color: "#ffffff" }}>
                  {arrow}
                </HoverBox>
              </span>
            </HoverBox>

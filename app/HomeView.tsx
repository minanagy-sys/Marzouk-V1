        ...
        <div style={{ maxWidth: 1240, margin: "0 auto" }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(270px, 1fr))", gap: 24, marginTop: 48 }}>
            {gridServices.map((svc) => (
              <HoverBox key={svc.slug} as={Link} href={`/services/${svc.slug}`} style={{ position: "relative", height: 460, borderRadius: 26, overflow: "hidden", border: "1px solid rgba(12,52,70,0.08)", boxShadow: "0 10px 30px rgba(12,52,70,0.08)", background: "linear-gradient(160deg, #0A3950, #0E5372)", transition: "all 0.35s ease", color: "#ffffff", display: "block" }} hoverStyle={{ transform: "translateY(-8px)", boxShadow: "0 28px 60px rgba(48,182,222,0.28)", color: "#ffffff" }}>
                ...
                <span style={{ position: "absolute", top: 18, insetInlineStart: 18, background: "rgba(48,182,222,0.92)", color: "#ffffff", borderRadius: 999, padding: "6px 16px", fontSize: 12, fontWeight: 800, letterSpacing: "0.6px", pointerEvents: "none", boxShadow: "0 6px 16px rgba(4,32,46,0.35)" }}>{pick(svc.tag, lang)}</span>
                <div style={{ position: "absolute", bottom: 0, left: 0, right: 0, padding: "28px 26px", pointerEvents: "none" }}>
                  <span style={{ fontFamily: SERIF, fontWeight: 700, fontSize: 24, lineHeight: 1.45, color: "#ffffff", display: "block" }}>{pick(svc.title, lang)}</span>
                </div>
              </HoverBox>
            ))}
          </div>

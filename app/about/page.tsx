...
export default function AboutPage() {
  ...
  const { t, facts, vm, why, testimonials } = aboutContent(lang);
      ...
      <section data-screen-label="Testimonials" style={{ padding: "0 24px 100px" }}>
        <div style={{ maxWidth: 1240, margin: "0 auto" }}>
          ...
          <div className="dam-3col" style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 26, marginTop: 50 }}>
            {testimonials.map((q) => (
              <div key={q.name} style={{ background: "#ffffff", border: "1px solid rgba(12,52,70,0.08)", borderRadius: 22, padding: "34px 30px", boxShadow: "0 4px 14px rgba(12,52,70,0.05)", display: "flex", flexDirection: "column", gap: 16 }}>
                <span style={{ fontFamily: SERIF, fontSize: 46, lineHeight: 0.6, color: "rgba(48,182,222,0.4)" }}>&#8220;</span>
                <p style={{ fontSize: 15, lineHeight: 2, color: "#46687A", margin: 0, flex: 1 }}>{q.text}</p>

            ...
            <div key={c.slug} className="dam-case" style={{ position: "sticky", top: 78, minHeight: "calc(100vh - 78px)", background: s.bg, boxShadow: "0 -30px 70px rgba(4,32,46,0.35)", display: "flex", flexWrap: "wrap", overflow: "hidden" }}>
              ...
              <div className="dam-casebody" style={{ flex: "1.1 1 380px", padding: "clamp(40px, 6vw, 100px) clamp(24px, 5vw, 90px)", display: "flex", flexDirection: "column", gap: 18, justifyContent: "center", position: "relative", overflow: "hidden", boxSizing: "border-box" }}>
                ...
                <div style={{ display: "flex", gap: 14, flexWrap: "wrap", marginTop: 8 }}>
                  <HoverBox as={Link} href={`/cases/${c.slug}`} style={{ background: "linear-gradient(135deg, #30B6DE, #1E92B8)", color: "#ffffff", borderRadius: 999, padding: "15px 34px", fontWeight: 800, fontSize: 15, boxShadow: "0 10px 26px rgba(48,182,222,0.4)" }} hoverStyle={{ boxShadow: "0 14px 34px rgba(48,182,222,0.55)", color: "#ffffff" }}>
                    {readMore}
                  </HoverBox>
                  <HoverBox as={Link} href="/contact" style={{ border: "1.5px solid " + (s.fg === "#ffffff" ? "rgba(255,255,255,0.4)" : "rgba(12,52,70,0.25)"), color: s.fg, borderRadius: 999, padding: "15px 30px", fontWeight: 700, fontSize: 15 }} hoverStyle={{ borderColor: "#30B6DE", color: "#30B6DE" }}>
                    {tc.book}
                  </HoverBox>
                </div>
              </div>

...
export default function CasesPage() {
  ...
  const { t, success, celebs } = casesContent(lang);
  const tc = common(lang);
  const [tab, setTab] = useState<"success" | "celebs">("success");

  useEffect(() => {
    if (typeof window !== "undefined" && window.location.hash === "#celebs") setTab("celebs");
    const onHash = () => {
      if (window.location.hash === "#celebs") setTab("celebs");
  ...
  const list = tab === "celebs" ? celebs : success;

  const tabBtn = (key: "success" | "celebs", label: string) => {
    ...
    <div style={{ fontFamily: SANS, direction: dir, color: "#0C3446", background: "#ffffff", minHeight: "100vh" }}>
      ...
      <PageHero crumbLabel={tc.navCases} title={t.pageTitle} sub={t.pageSub} spin={false} padding="90px 24px 70px">
        <div style={{ display: "flex", gap: 12, marginTop: 34, flexWrap: "wrap" }}>
          ...
          {tabBtn("celebs", t.tabCelebs)}
        </div>
      </PageHero>

      {/* CASES SHOWCASE (sticky scroll) */}
      <section data-screen-label="Cases showcase" style={{ background: "#04202E" }}>
        {list.map((c, i) => {
          const s = caseCardStyle(i, tab);
          return (
            <div key={s.slotId} className="dam-case" style={{ position: "sticky", top: 78, minHeight: "calc(100vh - 78px)", background: s.bg, boxShadow: "0 -30px 70px rgba(4,32,46,0.35)", display: "flex", flexWrap: "wrap", overflow: "hidden" }}>
              ...
              <div className="dam-casebody" style={{ flex: "1.1 1 380px", padding: "clamp(40px, 6vw, 100px) clamp(24px, 5vw, 90px)", display: "flex", flexDirection: "column", gap: 18, justifyContent: "center", position: "relative", overflow: "hidden", boxSizing: "border-box" }}>
                ...
                <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
                  ...
                  <span style={{ background: s.chipBg, color: s.chipFg, border: "1px solid rgba(48,182,222,0.4)", borderRadius: 999, padding: "6px 18px", fontSize: 12.5, fontWeight: 800, letterSpacing: "0.6px" }}>{c.tag}</span>
                </div>
                <div style={{ fontFamily: SERIF, fontWeight: 700, fontSize: "clamp(26px, 3.4vw, 42px)", lineHeight: 1.4, color: s.fg, textWrap: "pretty" }}>{c.title}</div>
                <p style={{ fontSize: "clamp(15px, 1.5vw, 17px)", lineHeight: 2.05, color: s.subC, margin: 0, maxWidth: 560, textWrap: "pretty" }}>{c.excerpt}</p>
                <HoverBox as={Link} href="/contact" style={{ alignSelf: "flex-start", marginTop: 8, background: "linear-gradient(135deg, #30B6DE, #1E92B8)", color: "#ffffff", borderRadius: 999, padding: "15px 34px", fontWeight: 800, fontSize: 15, boxShadow: "0 10px 26px rgba(48,182,222,0.4)" }} hoverStyle={{ boxShadow: "0 14px 34px rgba(48,182,222,0.55)", color: "#ffffff" }}>
                  {tc.book}
                </HoverBox>
              </div>
            </div>
